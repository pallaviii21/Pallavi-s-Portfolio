...
// Safe smooth scrolling for valid in-page anchor links (ignores "#" or empty targets)
...
function setupScrollAnimations() {
  ...
  document.querySelectorAll('.container-hero-image-landing, .homepage-text-section-landing, .about-card-home, .proj-item').forEach((el) => {
    observer.observe(el);
  });
}
...
// Button transition overlay to projects page
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (techStackWrapper) {
    const onScroll = () => {
      ...
      if (wrapperPosition < screenHeight * 0.85) {
